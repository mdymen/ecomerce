...
//        
        ...
        $info = array(
            'ID_LUTA_COM' => $p['id'],
            'ST_COMENTARIO_COM' => $p['comentario'],
            'ID_GANADOR_COM' => $p['ganador']
        );
        
        $result = $db->insert('luta_comentarios', $info);
...
//            VL_ESTADOFISICO_LUT
    ...
    function utimas_lutas($id) {
        $select = $this->_db->select()->from("luta")
            ...
            ->where("luta.ID_LUTADORUM_LUTA = ? ",$id)
            ->orWhere("luta.ID_LUTADORDOIS_LUTA = ? ", $id);
